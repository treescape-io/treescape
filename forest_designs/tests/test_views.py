...
class PlantViewTestCase(ForestDesignsViewTestMixin, TestCase):
    ...
    def test_plant_detail(self):
        ...
        response = self.client.get(
            reverse("plant-detail", kwargs={"id": self.plant.id})
        )
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
    ...
    def test_spatial_filter_api_is_functional(self):
        ...
        self.assertEqual(
            response.status_code, 200, "API should accept in_bbox parameter"
        )
    ...
    def test_format_combinations(self):
        ...
        Plant.objects.create(species=self.species, location="POINT(1 1)")
    ...
    def test_plant_state_update(self):
        ...
        self.assertEqual(
            PlantStateTransition.objects.filter(plant=self.plant).count(), 2
        )
...
class PlantStateViewTestCase(ForestDesignsViewTestMixin, TestCase):
    ...
    def test_create_plant_state(self):
        ...
        url = reverse("plantstate-list")
        data = {"name": unique_name, "description": "Plant has been removed or died"}
        response = self.client.post(
            url, data=json.dumps(data), content_type="application/json"
        )
        self.assertEqual(response.status_code, 201)
